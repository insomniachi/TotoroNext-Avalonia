...
internal static class Sig
{
    public static byte[] Write(int sampleRate, int sampleCount, PeakFinder finder)
    {
        ...
        writer.Write(0xCAFE2580); // Dialing "2580" on your phone and holding it up to the music, https://www.shazam.com/company
        writer.Write(-1);
        writer.Write(-1);
        ...
        while (mem.Position < mem.Length)
        {
            ...
            while (mem.Position < end)
            {
                if (end - mem.Position >= 5)
                {
                    ...
                    if (bin == 0 || magnitude == 0)
                    {
                        throw new InvalidOperationException();
                    }

                    writableBands[bandIndex].Add(new PeakInfo(stripe, bin, magnitude));
                }
                else
                {
    ...
    private static byte[][] GetBandData(PeakFinder finder)
    {
        finder.ApplyRateLimit();
        return finder.EnumerateBandedPeaks().Select(GetBandData).ToArray();
    }

    private static byte[] GetBandData(IEnumerable<PeakInfo> peaks)
    {
        using var mem = new MemoryStream();
        using var writer = new BinaryWriter(mem);
        var stripeIndex = 0;
    ...
    private static int CalcPadLen(int dataLen)
    {
    ...
    private static int SampleRateFromCode(int code)
    {
        code &= 0xf;
        return code switch
        {
    ...
    private static uint GetVersionCode()
    {
